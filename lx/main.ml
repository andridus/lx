let () =
  ...
  | Failure msg when String.contains msg '\'' ->
      ...
      if String.contains msg 't' && String.contains msg 'e' && String.contains msg 's' then
        Printf.eprintf "Error: 'test' is a reserved word and cannot be used as a function name.\nReserved words include: test, spec, describe, worker, supervisor, etc.\nTry using a different name like 'test_func' or 'my_test'.\n"
      else
  ...
  | exn ->
      ...
      if String.contains error_msg 'M' && String.contains error_msg 'e' && String.contains error_msg 'n' then
        Printf.eprintf "Syntax error: Unable to parse the input. This might be caused by:\n- Using reserved words like 'test', 'spec', 'describe' as function names\n- Incorrect syntax in your LX code\n\nTry using different function names or check your syntax.\n"
      else
