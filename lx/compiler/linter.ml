  ...
  | Receive (clauses, timeout_opt) -> (
      ...
      let errors =
        List.fold_left
          ...
          errors clauses
      in
      (* Lint timeout clause if present *)
...
and lint_guard_expr ctx errors guard =
  ...
  | GuardNot g -> lint_guard_expr ctx errors g
  ...
  | GuardCall (func, args) ->
      ...
      let errors =
        ...
        | "is_list" | "is_tuple" | "abs" | "round" | "trunc" | "hd" | "tl"
        | "length" | "element" ->
            errors
        | _ ->
            let error =
  ...
  | GuardAtom atom -> lint_guard_atom ctx errors atom

and lint_guard_atom ctx errors atom =
  match atom with
  | GuardVar var_name ->
      use_variable ctx var_name None;
      if (not (lookup_variable ctx var_name)) && not (is_ignored_var var_name)
      then
        let error =
          create_lint_error
            (UndefinedVariable (var_name, None))
