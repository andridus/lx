...
let string_of_otp_error = function
  | MissingRequiredHandler (handler, component) ->
      ...
      (match handler with
       | Init -> "init" | Call -> "call" | Cast -> "cast"
       | Info -> "info" | Terminate -> "terminate") ^
  ...
  | InvalidHandlerSignature (handler, component, expected, actual) ->
      ...
      (match handler with
       | Init -> "init" | Call -> "call" | Cast -> "cast"
       | Info -> "info" | Terminate -> "terminate") ^
      "' in component '" ^ component ^ "'. Expected: " ^ string_of_type expected ^
      ", but got: " ^ string_of_type actual
  | InvalidSupervisorStrategy strategy ->
      "Invalid supervisor strategy: " ^ strategy ^ ". Must be one of: one_for_one, one_for_all, rest_for_one"
  | InvalidChildSpecification (supervisor, child) ->
      "Invalid child specification for '" ^ child ^ "' in supervisor '" ^ supervisor ^ "'"
  ...
  | InvalidSupervisorName name ->
      "Invalid supervisor name: '" ^ name ^ "'. Supervisor names must be valid atoms"
...
let expected_handler_types = [
  (Init, TFun (TVar 1, TVar 2)); (* More flexible type for init *)
  (Call, TFun (TVar 3, TFun (TVar 4, TFun (TVar 5, TVar 6)))); (* More flexible type for call *)
...
let check_duplicate_handlers (handlers : (otp_handler * function_def) list) (component_name : string) =
  let handler_counts = List.fold_left (fun acc (handler, _) ->
    let count = try List.assoc handler acc with Not_found -> 0 in
    (handler, count + 1) :: (List.remove_assoc handler acc)
  ) [] handlers in

  List.iter (fun (handler, count) ->
    if count > 1 then
      raise (OtpValidationError (DuplicateHandler (handler, component_name)))
  ) handler_counts

(* Validate handler signatures - simplified for now *)
let validate_handler_signature (handler : otp_handler) (func : function_def) (component_name : string) (env : type_env) =
  ...
  with
  | TypeError _ ->
      Printf.printf "Warning: Handler %s in %s has type issues, but validation continues\n"
        (match handler with | Init -> "init" | Call -> "call" | Cast -> "cast" | Info -> "info" | Terminate -> "terminate")
        component_name;
...
let validate_worker (worker : otp_component) (env : type_env) =
  ...
  | Worker { name; handlers; functions; specs = _ } ->
      ...
      List.iter (fun required_handler ->
        if not (List.exists (fun (h, _) -> h = required_handler) handlers) then
          raise (OtpValidationError (MissingRequiredHandler (required_handler, name)))
      ) required_handlers_gen_server;

      (* Validate handler signatures *)
      List.iter (fun (handler, func) ->
  ...
  | Supervisor _ ->
      failwith "validate_worker called with supervisor component"
...
let build_dependency_graph (components : otp_component list) : (string * string list) list =
  ...
  let rec dfs visited path node =
    ...
    if List.mem node visited then
      ...
      List.fold_left (fun acc child ->
        dfs acc new_path child
      ) (node :: visited) children
  in

  let nodes = List.map fst graph in
  let _ = List.fold_left (fun visited node ->
    if List.mem node visited then visited
    else dfs visited [] node
  ) [] nodes in
  ()

(* Validate supervisor component *)
let validate_supervisor (supervisor : otp_component) (all_components : otp_component list) =
  ...
  | Supervisor { name; strategy; children } ->
      ...
      let component_names = List.map (function
        | Worker { name; _ } -> name
        | Supervisor { name; _ } -> name
      ) all_components in
  ...
  | Worker _ ->
      failwith "validate_supervisor called with worker component"
...
let validate_otp_application (components : otp_component list) (env : type_env) =
  ...
  let has_workers = List.exists (function Worker _ -> true | _ -> false) components in
  let has_supervisors = List.exists (function Supervisor _ -> true | _ -> false) components in

  if has_workers && not has_supervisors then
    Printf.printf "Warning: Application has workers but no supervisors. Consider adding a supervisor.\n";

  (* Validate supervisor tree structure *)
  let supervisors = List.filter_map (function
    | Supervisor { name; children; _ } -> Some (name, children)
    | _ -> None
  ) components in

  (* Check for orphaned components *)
  let all_children = List.fold_left (fun acc (_, children) -> children @ acc) [] supervisors in
  let component_names = List.map (function
    | Worker { name; _ } -> name
    | Supervisor { name; _ } -> name
  ) components in

  let root_supervisors = List.filter (fun name -> not (List.mem name all_children)) component_names in

  if List.length root_supervisors = 0 && List.length components > 0 then
    Printf.printf "Warning: No root supervisor found. All components are children of other supervisors.\n";

  if List.length root_supervisors > 1 then
    Printf.printf "Warning: Multiple root supervisors found: %s. Consider using a single application supervisor.\n"
...
let validate_program (program : program) (env : type_env) : unit =
  ...
  let otp_components = List.fold_left (fun acc item ->
    match item with
    | OtpComponent comp -> comp :: acc
    | _ -> acc
  ) [] program.items in
...
let is_valid_otp_behavior (component : otp_component) : bool =
  try
    match component with
    | Worker { handlers; _ } ->
        List.for_all (fun required ->
          List.exists (fun (h, _) -> h = required) handlers
        ) required_handlers_gen_server
    | Supervisor { strategy; _ } ->
        (match strategy with
         | OneForOne | OneForAll | RestForOne -> true)
  with
  | _ -> false