...
let rec string_of_type = function
  ...
  | TRecord fields ->
      "#{" ^ String.concat ", " (List.map (fun (k, v) -> k ^ ": " ^ string_of_type v) fields) ^ "}"
  | TOtpState -> "otp_state"
...
let string_of_type_error = function
  ...
  | OccursCheck (var, typ) ->
      "Occurs check failed: " ^ string_of_type (TVar var) ^ " occurs in " ^ string_of_type typ
  | HandlerTypeMismatch (handler, expected, actual) ->
      "Handler " ^ (match handler with
        | Init -> "init" | Call -> "call" | Cast -> "cast"
        | Info -> "info" | Terminate -> "terminate") ^
      " has wrong type. Expected: " ^ string_of_type expected ^
      ", but got: " ^ string_of_type actual
  | MissingHandler handler ->
      "Missing required handler: " ^ (match handler with
        | Init -> "init" | Call -> "call" | Cast -> "cast"
        | Info -> "info" | Terminate -> "terminate")
...
let rec apply_subst (subst : substitution) (typ : lx_type) : lx_type =
  match typ with
  | TVar var ->
      (try List.assoc var subst with Not_found -> typ)
  | TFun (t1, t2) ->
...
let rec unify (t1 : lx_type) (t2 : lx_type) : substitution =
  ...
  | (TBool, TBool) | (TAtom, TAtom) | (TUnit, TUnit)
  | (TOtpState, TOtpState) | (TOtpCast, TOtpCast) | (TOtpInfo, TOtpInfo) -> []
...
let rec infer_pattern (env : type_env) (pattern : pattern) : lx_type * type_env * substitution =
  match pattern with
  | PWildcard ->
      (fresh_type_var (), env, [])
  | PVar var ->
  ...
  | PAtom _ ->
      (TAtom, env, [])
  | PLiteral lit ->
      (infer_literal lit, env, [])
  | PTuple patterns ->
      let (types, new_env, substs) = List.fold_left (fun (types, env_acc, substs_acc) pat ->
  ...
  | PList patterns ->
      ...
      let (_, new_env, substs) = List.fold_left (fun (_, env_acc, substs_acc) pat ->
        let (typ, new_env, subst) = infer_pattern env_acc pat in
        let unify_subst = unify typ elem_type in
        (typ, new_env, compose_subst (compose_subst substs_acc subst) unify_subst)
      ) (elem_type, env, []) patterns in
  ...
  | PCons (head, tail) ->
      ...
      let final_subst = compose_subst (compose_subst subst1 subst2) unify_subst in
...
let rec infer_expr (env : type_env) (expr : expr) : lx_type * substitution =
  ...
  | Fun (params, body) ->
      ...
      let fun_type = List.fold_right (fun param_type acc -> TFun (param_type, acc)) param_types body_type in
      (fun_type, subst)
  | App (func, args) ->
      let (func_type, subst1) = infer_expr env func in
      let (arg_types, subst2) = List.fold_left (fun (types, subst_acc) arg ->
        let (arg_type, subst) = infer_expr (apply_subst_env subst_acc env) arg in
        (arg_type :: types, compose_subst subst_acc subst)
      ) ([], subst1) args in
      let arg_types = List.rev arg_types in
      let result_type = fresh_type_var () in
      let expected_func_type = List.fold_right (fun arg_type acc -> TFun (arg_type, acc)) arg_types result_type in
      let unify_subst = unify (apply_subst subst2 func_type) expected_func_type in
      let final_subst = compose_subst subst2 unify_subst in
      (apply_subst final_subst result_type, final_subst)
  | Tuple exprs ->
      let (types, subst) = List.fold_left (fun (types, subst_acc) expr ->
  ...
  | List exprs ->
      let elem_type = fresh_type_var () in
      let subst = List.fold_left (fun subst_acc expr ->
        let (typ, subst) = infer_expr (apply_subst_env subst_acc env) expr in
        let unify_subst = unify typ elem_type in
  ...
  | Match (value, cases) ->
      let (value_type, subst1) = infer_expr env value in
      let result_type = fresh_type_var () in
      let (final_subst, _) = List.fold_left (fun (subst_acc, env_acc) (pattern, case_expr) ->
        let (pattern_type, pattern_env, pattern_subst) = infer_pattern env_acc pattern in
        let unify_subst1 = unify (apply_subst subst_acc value_type) pattern_type in
        let combined_subst = compose_subst (compose_subst subst_acc pattern_subst) unify_subst1 in
        let case_env = pattern_env @ (apply_subst_env combined_subst env_acc) in
        let (case_type, case_subst) = infer_expr case_env case_expr in
        let unify_subst2 = unify (apply_subst case_subst result_type) case_type in
        let final_subst = compose_subst (compose_subst combined_subst case_subst) unify_subst2 in
  ...
  | If (cond, then_expr, else_expr) ->
      ...
      let (then_type, subst2) = infer_expr (apply_subst_env combined_subst1 env) then_expr in
      let combined_subst2 = compose_subst combined_subst1 subst2 in
      (match else_expr with
       | Some else_expr ->
           let (else_type, subst3) = infer_expr (apply_subst_env combined_subst2 env) else_expr in
           let else_unify = unify (apply_subst subst3 then_type) else_type in
           let final_subst = compose_subst (compose_subst combined_subst2 subst3) else_unify in
...
let builtin_env : type_env = [
  ("gen_server", TFun (TAtom, TFun (TAtom, TFun (TList TAtom, TFun (TList TAtom, TOtpState)))));
...
let type_check_program (program : program) : type_env =
  ...
  List.fold_left (fun env_acc item ->
    ...
    | Test desc ->
        ...
        List.fold_left (fun test_env test ->
