...
let test_valid_worker () =
  let init_handler = {
    name = "init";
    params = ["args"];
    body = App (Var "reply", [Tuple [Var "ok"; Literal (LAtom "state")]]);
  } in

  let call_handler = {
    name = "handle_call";
    params = ["msg"; "from"; "state"];
    body = App (Var "reply", [Var "msg"; Var "state"]);
  } in
  ...
  | TypeError error ->
      fail ("Valid worker test failed (type error): " ^ string_of_type_error error)
...
let test_missing_handler () =
  ...
  let worker = Worker {
    ...
    handlers = [(Init, init_handler)]; (* Missing Call handler *)
    functions = [];
    specs = [];
  } in
  ...
  | OtpValidationError (MissingRequiredHandler (Call, _)) ->
      ()
  | OtpValidationError error ->
      fail ("Missing handler test failed: wrong error: " ^ string_of_otp_error error)
  | TypeError _ ->
      fail "Missing handler test failed: type error occurred"
...
let test_invalid_worker_name () =
  ...
  let worker = Worker {
    name = "Invalid-Worker-Name"; (* Invalid atom name *)
    handlers = [(Init, init_handler); (Call, call_handler)];
    functions = [];
    specs = [];
  } in
  ...
  | OtpValidationError (InvalidWorkerName _) ->
      ()
  | OtpValidationError error ->
      fail ("Invalid worker name test failed: wrong error: " ^ string_of_otp_error error)
  | TypeError _ ->
      fail "Invalid worker name test failed: type error occurred"
...
let test_valid_supervisor () =
  ...
  let program = {
    items = [OtpComponent supervisor; OtpComponent worker]
  } in
  ...
  | TypeError error ->
      fail ("Valid supervisor test failed (type error): " ^ string_of_type_error error)

let test_unknown_child () =
  let supervisor = Supervisor {
    name = "test_supervisor";
    strategy = OneForOne;
    children = ["unknown_worker"]; (* Worker doesn't exist *)
  } in

  let program = {
    items = [OtpComponent supervisor]
  } in
  ...
  | OtpValidationError (UnknownChild (_, _)) ->
      ()
  | OtpValidationError error ->
      fail ("Unknown child test failed: wrong error: " ^ string_of_otp_error error)
  | TypeError _ ->
      fail "Unknown child test failed: type error occurred"
