...
let test_simple_guard_parsing () =
  let input = "pub fun test(x) when x > 0 { x }" in
...
let test_guard_with_type_test () =
  let input = "pub fun test(x) when is_atom(x) { x }" in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
...
let test_guard_with_and () =
  let input = "pub fun test(x, y) when x > 0 and y < 10 { x + y }" in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
...
let test_guard_with_or () =
  let input = "pub fun test(x) when x > 0 or x < -10 { x }" in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
...
let test_guard_with_not () =
  let input = "pub fun test(x) when not is_atom(x) { x }" in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
...
let test_case_guard () =
  let input =
    "pub fun test(x) { case x { n when n > 0 -> :positive _ -> :other } }"
  in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
  let contains_when = string_contains_substring result " when " in
...
let test_operator_conversion () =
  let input = "pub fun test(x) when x != 5 and x <= 10 { x }" in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
...
let test_guard_andalso () =
  let input = "pub fun test(x, y) when x > 0 andalso y < 100 { :valid }" in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
...
let test_guard_orelse () =
  let input = "pub fun test(x, y) when x > 100 orelse y < 0 { :valid }" in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
...
let test_guard_mixed_operators () =
  let input =
    "pub fun test(x, y, z) when x > 0 and (y < 10 orelse z > 100) { :valid }"
  in
  let program = Compiler.parse_string input in
  let result = Compiler.compile_to_string_for_tests program in
