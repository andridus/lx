...
let test_simple_function_example () =
  let program = Compiler.parse_string "def hello() do \"world\" end" in
...
let test_function_with_parameters () =
  let program = Compiler.parse_string "def add(x, y) do x + y end" in
  let result = Compiler.compile_to_string_for_tests program in
  let expected_parts =
...
let test_multiple_functions () =
  let input = "def first() do 1 end def second() do 2 end" in
...
let test_various_literals () =
  let test_cases =
    [
      ("def test_int() do 42 end", [ "42" ]);
      ("def test_float() do 3.14 end", [ "3.14" ]);
      ("def test_bool() do true end", [ "true" ]);
      ("def test_atom() do :hello end", [ "hello" ]);
      ("def test_string() do \"world\" end", [ "\"world\"" ]);
    ]
  in
